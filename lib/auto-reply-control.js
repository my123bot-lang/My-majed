...
function normalizeCommand(text) {
  return normalizeText(text)
    .toLowerCase()
    // علامات واتساب الخفية واتجاه النص التي تمنع تطابق stop/Stop
    .replace(/[\u200B-\u200F\u202A-\u202E\u2066-\u2069\uFEFF\u00A0]/g, "")
    .replace(/^[/!.,:;~\-]+/, "")
    .replace(/[/!.,:;~\-]+$/, "")
    .replace(/\s+/g, " ")
...
function fileMtimeMs(accountId) {
  try {
    return fs.statSync(pausedPathForAccount(accountId)).mtimeMs;
  } catch (_) {
    return null;
  }
}
...
function persistPausedForCurrentAccount() {
  ...
  try {
    ...
    loadedMtimeMs = fileMtimeMs(accountId);
  } catch (err) {
...
function ensureLoaded() {
  const accountId = getCurrentWaAccountId() || "default";
  const mtime = fileMtimeMs(accountId);
  // أعد التحميل عند تغيّر الحساب أو الملف (مشاركة الحالة بين البوت واللوحة)
  if (loadedAccountId !== accountId || mtime !== loadedMtimeMs) {
    loadedAccountId = accountId;
...
function parseOwnerCommand(text) {
  ...
  // عناوين أزرار قائمة واتساب (للمالك فقط) + صيغ شائعة
  const stopPhrases = [
    "owner_pause_reply",
    "pause_auto_reply",
    "ايقاف الرد",
  ...
  if (stopList.includes(cmd) || stopPhrases.includes(cmd)) return "stop";
  if (startList.includes(cmd) || startPhrases.includes(cmd)) return "start";

  return null;
}

function isOwnerCommandMessageSeen(msg) {
  const id = msg?.id?._serialized || msg?.id;
  if (!id) return false;
  return recentOwnerCommandIds.has(id);
}

function markOwnerCommandMessageSeen(msg) {
  const id = msg?.id?._serialized || msg?.id;
  if (!id) return;
...
/** @deprecated استخدم isOwnerCommandMessageSeen + markOwnerCommandMessageSeen */
function rememberOwnerCommandMessage(msg) {
  if (isOwnerCommandMessageSeen(msg)) return true;
  markOwnerCommandMessageSeen(msg);
  return false;
}
...
function isChatPaused(chatId, options = {}) {
  ensureLoaded();
  if (!chatId && !(options.extraKeys || []).length) return false;
  const extras = Array.isArray(options.extraKeys) ? options.extraKeys : [];
  const keys = expandChatKeys(chatId, ...extras);
...
/**
 * فحص الإيقاف مع دمج مفاتيح الجلسة النشطة (@lid + رقم الجوال).
 * @param {string} chatId
 * @param {{ extraKeys?: string[] }} [options]
 */
function isChatPausedForIdentity(chatId, options = {}) {
  const sessionStore = require("./session");
  const extras = [
    ...(Array.isArray(options.extraKeys) ? options.extraKeys : []),
    ...sessionStore.collectIdentityKeys(chatId),
    ...sessionStore.findRelatedIdentityKeys(
      chatId,
      ...(Array.isArray(options.extraKeys) ? options.extraKeys : [])
    ),
  ];
  return isChatPaused(chatId, { extraKeys: extras });
}
...
module.exports = {
  ...
  isOwnerCommandMessageSeen,
  markOwnerCommandMessageSeen,
  statusLabel,
  pausedChatsCount,
  resumeAllChats,
  pauseChat,
