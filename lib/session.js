...
/** آخر أمر بدء (مرحبا/هلا/1…) — منفصل عن الرسائل العادية حتى لا تبلعها رسالة معاد توجيهها */
/** @type {Record<string, number>} */
...
function isResetCommand(text) {
  ...
  const normalized = normalizeResetKeyword(normalizeText(text));
...
function isStartCommandText(text) {
  const { isPoliteGreeting, matchesStartKeyword } = require("./validators");
  const { restartKeywords: restart, menuStartKeywords: menuStart } =
    CONFIG.session;
  if (isPoliteGreeting(text)) return true;
  if (matchesStartKeyword(text, restart || [])) return true;
  if (matchesStartKeyword(text, menuStart || [])) return true;
  return isResetCommand(text);
}
...
function shouldThrottle(from, text) {
  ...
  // أرقام القائمة أثناء جلسة مفتوحة لا تُحظر — قد يتكرر «1» في خطوة لاحقة
  ...
  // مرحبا / هلا / السلام عليكم / 1 لا تسقط بسبب رسالة سابقة ليست أمراً
  // (مثل رسالة معاد توجيهها وصلت قبلها بجزء من الثانية)
  if (isStartCommandText(text)) {
    const lastCmd = lastCommandTime[from];
    if (lastCmd && now - lastCmd < spamDelayMs) {
      return true;
    }
    lastCommandTime[from] = now;
    lastMessageTime[from] = now;
...
function findSessionByPhone(phone) {
  ...
  for (const [chatId, session] of Object.entries(sessions)) {
    ...
    const hit = candidates.some((c) => {
      const d = digitsOnly(c).replace(/^966/, "").replace(/^0+/, "");
      return d && (d === needle || d.endsWith(last9) || last9.endsWith(d.slice(-9)));
    });
    if (hit) return { chatId, session };
  }
  return null;
}
