...
/** أزرار واضحة للمالك فقط — تظهر مباشرة بدون تمرير القائمة */
function ownerAutoReplyControls() {
  return {
    kind: "buttons",
    body: "تحكم الرد الآلي (ظاهر لك فقط):\nإيقاف = آخر عميل راسل البوت",
    buttons: [
...
function inquiryMain(options = {}) {
  ...
  return {
    ...
    body: options.ownerControls
      ? "وعليكم السلام ورحمه الله وبركاته\n\nمرحبا معك ماجد.\nمانوع استفسارك؟\n\n(خيارات إيقاف/تشغيل الرد الآلي ظاهرة لك فقط)"
      : "وعليكم السلام ورحمه الله وبركاته\n\nمرحبا معك ماجد.\nمانوع استفسارك؟",
    buttonText: "الخيارات",
    rows,
  };
...
function companySearchPrompt() {
  return {
    kind: "buttons",
    body: require("../config").messages.companySearchPrompt,
    buttons: [{ id: "co_ok", title: "حسناً" }],
  };
}
...
function loanTermYears(years = [1, 2, 3, 4, 5]) {
  const labels = {
    1: "سنة",
    2: "سنتين",
    3: "3 سنوات",
...
function loanTermYearsInvalid(years) {
  return {
    kind: "list",
    body: "الرجاء اختيار مدة التمويل من 1 إلى 5 سنوات:",
    buttonText: "المدة",
    rows: loanTermYears(years).rows,
  };
}
