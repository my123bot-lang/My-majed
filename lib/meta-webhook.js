...
/**
 * رسائل صادرة من تطبيق الأعمال (coexistence / message echoes) — لأوامر stop/start.
 * @returns {Array<{phone, messageId, body, type, timestamp}>}
 */
  ...
  for (const entry of payload.entry || []) {
    for (const change of entry.changes || []) {
      ...
      const echoes =
        ...
        (field.includes("echo") ? value.messages : null) ||
        [];

      for (const msg of echoes) {
        // في الـ echoes: to = العميل غالباً
