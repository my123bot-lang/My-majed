 ...
 * بما فيها smb_message_echoes (رسائل تطبيق واتساب الأعمال عبر Coexistence)
 */
...
function extractMetaMessageBody(msg) {
  let body = "";
  ...
  for (const entry of payload.entry || []) {
    for (const change of entry.changes || []) {
      ...
      for (const msg of value.messages || []) {
        ...
        const { body, type } = extractMetaMessageBody(msg);
...
/**
 * رسائل صادرة من تطبيق واتساب الأعمال (Coexistence → smb_message_echoes).
 * هذا المسار الوحيد الموثوق على السحابة لما تكتب stop من التطبيق نفسه.
 * @returns {Array<{phone, messageId, body, type, timestamp, from}>}
 */
function parseOwnerEchoMessages(payload) {
  if (!payload || payload.object !== "whatsapp_business_account") return [];
    ...
    for (const change of entry.changes || []) {
      ...
      const echoes = Array.isArray(value.message_echoes)
        ...
        : Array.isArray(value.smb_message_echoes)
          ...
          : isEchoField && Array.isArray(value.messages)
            ? value.messages
            : [];
      ...
      for (const msg of echoes) {
        // to = العميل؛ from = رقم البوت/العمل
        const phone = String(msg.to || msg.recipient || "").replace(/\D/g, "");
        if (!phone) continue;
        const { body, type } = extractMetaMessageBody(msg);
        if (!body) continue;
        out.push({
          ...
          type,
          timestamp: msg.timestamp || null,
          from: String(msg.from || "").replace(/\D/g, "") || null,
          field: field || "smb_message_echoes",
        });
      }
    }
  }
...
function describeMetaWebhook() {
  const cfg = getConfig();
  return {
    sendConfigured: Boolean(cfg.token && cfg.phoneNumberId),
    verifyTokenSet: Boolean(cfg.verifyToken),
    appSecretSet: Boolean(cfg.appSecret),
    echoesPath: "/webhooks/meta",
    requiredField: "smb_message_echoes",
    usage:
      "اشترك في smb_message_echoes على Meta ووجّه Webhook إلى /webhooks/meta حتى يعمل stop من تطبيق واتساب الأعمال",
  };
}
