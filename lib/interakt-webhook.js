...
function parseInboundMessage(payload) {
  ...
  if (
    ...
    body = String(
      buttonId || message.message || message.text || message.button_text || ""
    ).trim();
  }

  // بعض الردود التفاعلية تصل كنص JSON
  if (body.startsWith("{") && body.includes("id")) {
    try {
      const parsed = JSON.parse(body);
      body = String(parsed.id || parsed.title || parsed.text || body).trim();
    } catch (_) {
      /* ignore */
    }
  }
