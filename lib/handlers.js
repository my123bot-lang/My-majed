...
async function handleIncomingMessage(msg) {
  ...
  if (isOwnerControlMessage(msg, session)) {
    const fromPhone =
      ...
      msg.from;
    const ownerSend = async (_chatId, body) => {
      await replyToMessage(msg, body);
    };

    // اليوم / احصائية / stop / start — من رقم التحكم
    if (await tryHandleOwnerRemoteControl(fromPhone, text, { send: ownerSend })) {
      return true;
    }
    ...
    await replyToMessage(
      msg,
      "رقم جوالك غير مسجّل للتحكم بإيقاف الرد الآلي.\n\nمن لوحة التحكم → الإعدادات → «أرقام التحكم بإيقاف الرد»\nأضف رقم واتساب الذي تراسل منه (مثل رقم ماجد للتمويل)، احفظ، ثم أعد إرسال: تحكم"
    );
    return true;
  }
