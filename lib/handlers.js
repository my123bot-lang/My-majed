 ...
 *    مشترك: قطاع → راتب → عقاري → التزامات → عرض مبلغ → مبلغ أقل → مدة (1–5) → طريقة التقديم
...
/** إعادة سؤال الرقم الحالي (راتب / دعم / التزامات) بدون مسح الجلسة */
async function repromptNumberEntryStep(msg, session) {
  ...
  if (session.step === "support_amount") {
    await replyToMessage(msg, messages.supportAmountMessage());
    return true;
  }

  if (session.step === "commitments") {
...
async function handleIncomingMessage(msg) {
  ...
  // عند الراتب / الدعم / الالتزامات: «إعادة» تعيد نفس السؤال دون إلغاء الجلسة
  if (sessionStore.isResetCommand(text)) {
    if (session && (await repromptNumberEntryStep(msg, session))) {
      return true;
    }
  ...
  if (restartKeywords.includes(text)) {
    return openMainMenu(msg, from);
  }
...
async function handleLoanTermPick(msg, from, session, text) {
  const allowed = loanTermYearsOptions();
  const years = parseLoanTermYears(text, allowed);

  if (years == null) {
    await replyMenu(msg, menus.loanTermYearsInvalid(allowed));
    return true;
  }
