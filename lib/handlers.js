 ...
 *
 * تدفقات:
 * 1) inquiry_type → 5 خيارات (شخصي | مديونية | إيقاف خدمات | دوام | موقع)
 * 2) المسار 3 (إيقاف خدمات): شروط → رقم المندوب (أبو تركي…) + تسجيل في السجل
 * 3) المسار 1 (شخصي) — جميع القطاعات (تفاصيل في config.personalFinancingPath):
 *    مشترك: قطاع → راتب → عقاري → التزامات → عرض مبلغ → مبلغ أقل → طريقة التقديم
 ...
 * 5) إيقاف خدمات: رسالة الشروط + رقم المندوب + تسجيل السجل
 */
...
async function continueAfterEarlySalary(msg, session) {
  if (session.jobCategory === "military" && needsMilitaryRemoteAllowanceCheck(session)) {
    ...
    await replyToMessage(msg, messages.remoteAllowanceCheckMessage());
    return;
  }
...
async function continueAfterRemoteAllowance(msg, session) {
  if (isDebtFlow(session)) {
    ...
    await replyToMessage(msg, messages.realEstateMessage());
    return;
  }
  session.step = "real_estate";
  await replyToMessage(msg, messages.realEstateMessage());
...
const { restartKeywords } = CONFIG.session;
...
async function handleIncomingMessage(msg) {
  ...
  if (
    sessionStore.isResetCommand(text) ||
    restartKeywords.includes(text)
  ) {
    sessionStore.resetUser(from);
    await replyToMessage(msg, messages.startMessage());
    return true;
  }
  ...
  if (sessionStore.isClosed(from)) {
    if (text === "1" && sessionStore.tryRetry(from)) {
      await replyToMessage(msg, messages.startMessage());
      return true;
    }

    const closed = sessionStore.getClosedState(from);
    if (closed?.type === "success") {
      if (extractOrderNumber(text)) {
        console.warn(
          "[رقم طلب] وُجد في الرسالة لكن لم يُحفظ —",
          from,
          text.slice(0, 40)
        );
        return true;
      ...
      const postClose = sessionStore.handlePostSuccessMessage(from);
      if (postClose.shouldRestart) {
        sessionStore.resetUser(from);
        await replyToMessage(msg, messages.startMessage());
      }
      return true;
  ...
  if (!isTextLikeMessage(msg)) {
    if (!session) {
      ...
      session = sessionStore.getSession(from);
      await attachPhoneToSession(msg, session);
      await replyToMessage(msg, messages.startMessage());
      return true;
    }
  ...
  switch (session.step) {
    ...
    default:
      // خطوة غير معروفة — إعادة ضبط آمنة دون كسر الحساب
      sessionStore.startSession(from);
      await replyToMessage(msg,messages.startMessage());
      return true;
  }
...
async function handleInquiryType(msg, session, text) {
  ...
  if (choice === "assistant_contact" && !allowsAssistantContact()) {
    await replyToMessage(msg, messages.invalidInquiryTypeMessage());
    return true;
  }
  ...
  if (choice === "personal") {
    ...
    await replyToMessage(msg,messages.jobTypeMessage());
    return true;
  }
  ...
  if (choice === "service_stop") {
    ...
    await replyToMessage(msg, messages.serviceStopWelcomeMessage());
  ...
  if (choice === "pause_auto_reply") {
    callStats.recordInquiryType("pause_auto_reply");
    autoReplyControl.pauseChat(msg.from);
    sessionStore.clearSession(msg.from);
    await replyToMessage(msg, messages.pauseChatAutoReplyMessage());
    console.log("الرد الآلي موقوف لهذه المحادثة:", msg.from);
    return true;
  }
...
async function handleJobType(msg, session, text) {
  ...
  await replyToMessage(msg, messages.invalidJobTypeMessage());
  return true;
}
...
async function handleRemoteAllowanceCheck(msg, session, text) {
  ...
  await replyToMessage(msg, messages.invalidRemoteAllowanceCheckMessage());
  return true;
}
...
async function beginPersonalResult(msg, session, amount) {
  ...
  await replyToMessage(msg, messages.personalAmountOfferMessage(session));
}
...
async function handleRealEstate(msg, session, text) {
  ...
  await replyToMessage(msg,messages.invalidRealEstateMessage());
  return true;
}
...
async function handleSalary(msg, from, session, text) {
  ...
  if (!grossSalary || grossSalary <= 0 || Number.isNaN(grossSalary)) {
...
async function handlePropertyComboOffer(msg, from, session, text) {
  ...
  await replyToMessage(msg, messages.invalidPropertyComboMessage());
  return true;
}
...
async function promptApplicationMethod(msg, session) {
  ...
  await replyToMessage(msg, messages.applicationMethodMessage());
}
...
async function handleLowerAmountChoice(msg, from, session, text) {
  if (isYes(text)) {
    ...
    session.lowerAmountTiers = tiers;
    session.step = "lower_amount_pick";
    await replyToMessage(
      msg,
      messages.lowerAmountsNumberedListMessage(session, tiers)
    );
    return true;
  }
  ...
  await replyToMessage(msg, messages.invalidLowerAmountMessage());
  return true;
}
...
async function handleLowerAmountPick(msg, from, session, text) {
  ...
  if (!choice) {
    await replyToMessage(msg, messages.invalidLowerAmountPickMessage());
    return true;
  }

  const amount = tiers[choice - 1];
  session.selectedAmount = amount;
  await replyToMessage(msg, messages.selectedAmountDetailMessage(session, amount));
  await promptApplicationMethod(msg, session);
  return true;
}
...
async function handleApplicationMethod(msg, from, session, text) {
  ...
  await replyToMessage(msg, messages.invalidApplicationMethodMessage());
  return true;
}
...
async function handleDebtContinue(msg, from, session, text) {
  ...
  await replyToMessage(msg, messages.invalidDebtContinueMessage());
  return true;
}
...
async function handleContactEmployee(msg, from, session, text) {
  ...
  await replyToMessage(msg, messages.invalidContactEmployeeMessage());
  return true;
}
