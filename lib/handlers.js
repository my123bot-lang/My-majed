...
async function handleIncomingMessage(msg) {
  ...
  // مرحبا / هلا / السلام عليكم → نفس السلوك (و«1» خارج الجلسة عبر المسار أدناه)
...
async function handleLoanTermPick(msg, from, session, text) {
  ...
  if (!isLoanTermAffordable(session, amount, years, rate, jobCategory)) {
    const maxAllowed = resolveMaxAffordableLoanTermYears(
      session,
      amount,
      rate,
      jobCategory,
    ...
    finalYears = maxAllowed;
    session.loanTermYears = finalYears;
    session.loanTermMonths = finalYears * 12;
    await replyToMessage(
      msg,
      messages.allowedLoanTermNoticeMessage(finalYears)
    );
  } else {
    session.loanTermYears = finalYears;
    session.loanTermMonths = finalYears * 12;
