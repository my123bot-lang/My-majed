...
async function handleIncomingMessage(msg) {
  ...
  if (
    ...
    await replyMenu(msg, menus.inquiryMain());
    return true;
  }
...
async function handleInquiryType(msg, session, text) {
  ...
  if (choice === "assistant_contact" && !allowsAssistantContact()) {
    await replyMenu(msg, menus.inquiryInvalid());
    return true;
  }
  ...
  if (choice === "service_stop") {
    ...
    sessionStore.closeSuccess(msg.from);
    return true;
  }
...
async function beginPersonalResult(msg, session, amount) {
  ...
  await replyMenu(msg, menus.yesNo("هل تبي مبلغ أقل؟"));
}
...
async function handleLowerAmountChoice(msg, from, session, text) {
  if (isYes(text)) {
    ...
    await replyMenu(
      msg,
      menus.lowerAmountTiers(
        tiers.map((a) => Number(a).toLocaleString("en-US"))
      )
    );
    return true;
  }
