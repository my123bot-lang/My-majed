...
async function handleCompanySearch(msg, from, session, text) {
  ...
  await replyMenu(msg, menus.companyResearchAgain());
  return true;
}
...
function wantsCompanyResearch(text) {
  ...
  if (t === "نعم" || t.includes("نعم")) return true;
  if (
...
async function handleCompanyPick(msg, from, session, text) {
  ...
  // رد «لا» على سؤال إعادة البحث — أعد عرض قائمة الشركات
  if (raw === "لا") {
    await replyMenu(msg, menus.companyMatches(options));
    await replyMenu(msg, menus.companyResearchAgain());
    return true;
  }
