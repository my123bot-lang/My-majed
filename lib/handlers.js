...
function isMenuStartTrigger(text) {
  ...
  const keywords = menuStartKeywords || ["مرحبا", "هلا", "السلام عليكم", "1"];
...
async function handleIncomingMessage(msg) {
  ...
  // stop / start من العميل في أي خطوة
  if (await tryHandleCustomerChatControl(msg)) {
    return true;
  }
  ...
  // قائمة رئيسية فقط: مرحبا / هلا / السلام عليكم (و«1» خارج الجلسة)
  if (sessionStore.isResetCommand(text) || restartKeywords.includes(text)) {
...
async function handleInquiryType(msg, session, text) {
  ...
  if (choice === "pause_auto_reply") {
    ...
    const extraKeys = sessionStore.findRelatedIdentityKeys(
      msg.from,
      session?.whatsappNumber,
      session?.phoneDisplay,
      session?.chatId
    );
    autoReplyControl.pauseChat(msg.from, { extraKeys });
    sessionStore.clearSessionsMatching(msg.from, ...extraKeys);
...
async function promptCompanySearch(msg) {
  // إرسال تفاعلي أولاً (أوثق على Interakt من النص الحر أحياناً)
  try {
    await replyMenu(msg, menus.companySearchPrompt());
    return;
  } catch (err) {
    console.warn("[شركة] فشل برومبت تفاعلي:", err.message);
  }
...
async function handleCompanySearch(msg, from, session, text) {
  ...
  // ضغط زر حسناً / إعادة إرسال قطاع خاص بالخطأ → أعد طلب اسم الشركة
  if (
    !query ||
    query.length < 2 ||
    query === "co_ok" ||
