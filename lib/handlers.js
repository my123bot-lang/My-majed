...
async function handleIncomingMessage(msg) {
  ...
  switch (session.step) {
    ...
    case "salary_policy":
      return handleSalaryPolicyChoice(msg, from, session, text);
...
async function handleInquiryType(msg, session, text) {
  ...
  if (choice === "salary_policy") {
    ...
    session.step = "salary_policy";
    await replyMenu(msg, menus.salaryPolicy());
    return true;
  }
...
async function handleSalaryPolicyChoice(msg, from, session, text) {
  const choice = parseSalaryPolicyChoice(text);
  if (!choice) {
    await replyMenu(msg, menus.salaryPolicyInvalid());
    return true;
  }

  await replyToMessage(msg, messages.salaryPolicyDetailMessage(choice));
  sessionStore.resetUser(from);
  return true;
}
