...
function personalAmountOfferMessage(session) {
  ...
  const months = resolveLoanTermMonths(session);
  const termYears =
    session?.loanTermYears > 0
      ? Number(session.loanTermYears)
  ...
  const total = calculateTotalRepayment(roundedAmount, rate, months);
  const body = templates.personalAmountOffer(
    formatMoney(roundedAmount),
    formatMoney(installment),
    formatLoanTermLabel(termYears),
    formatMoney(total)
  );
...
function formatLoanTermLabel(years) {
...
function allowedLoanTermNoticeMessage(years) {
  return templates.allowedLoanTermNotice(formatLoanTermLabel(years));
}

function resolveLoanTermMonths(session) {
  if (session?.loanTermMonths > 0) return Number(session.loanTermMonths);
  if (session?.loanTermYears > 0) return Number(session.loanTermYears) * 12;
  return CONFIG.financing.loanTermMonths || 60;
}
...
function captureContactOnSession(session, deliveryKind) {
  ...
  switch (deliveryKind) {
    case "electronic":
      ...
      break;
    case "combo_agent": {
      name = settings.propertyComboAgentName || "المسؤول العقاري";
      const comboPhones = resolvePropertyComboPhones(session);
      phone = [comboPhones.phone1, comboPhones.phone2]
        .filter(Boolean)
...
function branchVisitMessage() {
  const fn = templates.branchVisitInfo;
  if (typeof fn === "function") return fn();
  return `موقعنا/حائل معرض السديري للسيارات
عند حضورك اسأل عن الموظف ماجد`;
}
...
function serviceStopQualifyQuestionMessage() {
  const fn =
    templates.serviceStopQualifyQuestion ||
    (() => "هل راتبك الأساسي 7000 وما عليك عقاري؟");
  return fn();
}

function serviceStopPackageMessage() {
  const build =
    templates.serviceStopPackageOffer || templates.propertyComboOffer;
  const pkg = CONFIG.comboPackage;
  return build(
    formatMoney(pkg.totalExample),
    formatMoney(pkg.propertyAmount),
...
function serviceStopTwoAgentsMessage() {
  const phones =
    CONFIG.financing.serviceStopContactPhones ||
    ["0506279834", "0546473109"];
  const fn =
    templates.serviceStopTwoAgents ||
    ((a, b) => `للتواصل:\n${a}\n${b}\n\nمن طرف ماجد`);
  return fn(phones[0], phones[1] || phones[0]);
}
...
function resolvePropertyComboPhones(session) {
  const settings = getContactSettings(session);
  const defaults =
    CONFIG.financing.propertyComboAgentPhones ||
    ["0506279834", "0546473109"];
  const phone1 =
    ...
    "0506279834";
  const phone2 =
    settings.propertyComboAgentPhone2 ||
    defaults[1] ||
    phone1;
...
function propertyComboAgentMessage(session) {
  const settings = getContactSettings(session);
  const { phone1, phone2 } = resolvePropertyComboPhones(session);
  const footer =
    settings.propertyComboContactFooter ||
    CONFIG.financing.propertyComboContactFooter ||
    "من طرف ماجد\nبالتوفيق ربي ييسر أمرك";
  return templates.propertyComboAgentDirect(phone1, phone2, footer);
}
...
function salaryPolicyDetailMessage(category) {
  ...
  const label = labels[category] || "القطاع";
  const amount = formatMoney(mins[category] || 0);
  return withInquiryFooter(templates.salaryPolicyDetail(label, amount));
}
...
module.exports = {
  ...
  serviceStopQualifyQuestionMessage,
  serviceStopPackageMessage,
  serviceStopTwoAgentsMessage,
  ...
  allowedLoanTermNoticeMessage,
  invalidLowerAmountMessage: () => messages.invalidLowerAmount,
  invalidLowerAmountPickMessage: () => messages.invalidLowerAmountPick,
  afterSalesEarlyPayoffMessage: () =>
    messages.afterSalesEarlyPayoff ||
    `تم اختيار: سداد مبكر

أرسل البيانات التالية:
1- الاسم الثلاثي
2- رقم الهوية
3- رقم الجوال المسجل لدينا`,
  afterSalesCollectionIssueMessage: () =>
    messages.afterSalesCollectionIssue ||
    `تم اختيار: مشكلة بالتحصيل

أرسل:
1- اسمك الثلاثي
2- سجلك المدني
3- اشرح مشكلتك`,
  afterSalesComplaintMessage: () =>
    messages.afterSalesComplaint ||
    `تم اختيار: شكاوى

أرسل:
1- اسمك الثلاثي
2- سجلك المدني`,
  afterSalesDetailsReceivedMessage: () =>
    messages.afterSalesDetailsReceived ||
    `تم استلام طلبك، بيتواصل معك الموظف ماجد قريباً.
رقم التواصل: 0507009290`,
  companySearchPromptMessage: () =>
    messages.companySearchPrompt ||
    `أرسل اسم الشركة للبحث في قائمة الشركات المعتمدة
أو جزء منها.

مثال:
ارامكس، المراعي، ارامكو، سابك، بنك الاهلي`,
  companyNotFoundMessage: () =>
    messages.companyNotFound ||
    `نعتذر منك، الشركة غير موجودة ضمن قائمة الشركات المعتمدة.`,
  companyApprovedMessage: (companyName) => {
    const fn = messages.companyApproved;
    if (typeof fn === "function") return fn(companyName);
    return `تم اعتماد الجهة:\n${companyName}`;
  },
