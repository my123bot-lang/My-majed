...
function getContactSettings(session) {
  const waId = session?.waAccountId || getCurrentWaAccountId() || "0488";
...
function employeeContactMessage(session) {
  ...
  if (isServiceStopSession(session)) {
    const attribution =
      settings.serviceStopContactHint || "من طرف رائد الحربي";
