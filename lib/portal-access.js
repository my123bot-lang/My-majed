/**
 * روابط بوابة خارجية — لوحة مخصصة لجوال ماجد
  ...
  try {
    if (fs.existsSync(STORE_PATH)) {
      ...
      if (Array.isArray(raw.portals) && raw.portals.length) {
        const REMOVED = new Set(["raied", "abdulrahman"]);
        const portals = raw.portals.filter(
          (p) =>
            !REMOVED.has(String(p.slug || "").toLowerCase()) &&
            p.waAccountId !== "0488" &&
            p.waAccountId !== "wa_1780305984859"
        );
        if (!portals.length) {
          const store = {
        ...
        const store = { updatedAt: raw.updatedAt || null, portals };
        if (portals.length !== raw.portals.length) saveStore(store);
        return store;
      }
    }
  } catch (err) {
