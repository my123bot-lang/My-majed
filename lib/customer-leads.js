...
function matchesCrmDay(row, day, today = todayKey(), yesterday = shiftDateKey(todayKey(), -1)) {
  ...
  return resolveOutcomeId(row) === day;
}
...
function computeTabCounts(rows) {
  ...
  for (const row of rows) {
    ...
    counts.all += 1;
    const key = leadDateKey(row);
    if (key === today) counts.today += 1;
    if (key === yesterday) counts.yesterday += 1;
    const outcomeId = resolveOutcomeId(row);
    if (outcomeId && counts[outcomeId] !== undefined) counts[outcomeId] += 1;
  }
  return counts;
}
...
function getFollowUpSafeSettings() {
  ...
  const dailyLimit = Math.max(Number(followUp.dailyLimit) || 80, 1);
  const minDelayMs = Math.max(Number(followUp.minDelayMs) || 8000, 8000);
  const maxBatchSize = Math.min(Math.max(Number(followUp.maxBatchSize) || 30, 1), 30);
