...
/** سجلات قبل هذا التاريخ بدون وسْم → تُحسب لرايد (بوت واحد سابقاً) */
...
function buildRecord(from, session, event) {
  ...
  if (session?.inquiryType === "إيقاف خدمات" && !contactAgentPhone) {
    ...
    const settings = loadSettingsForAccount(
      session?.waAccountId || waMeta.waAccountId || "0488"
    );
...
const DEFAULT_LEGACY_WA = { waAccountId: "0488", waAccountLabel: "رايد" };
...
function normalizeLeadRow(row) {
  ...
  if (next.inquiryType === "إيقاف خدمات") {
    ...
    if (!next.contactAgentPhone) {
      const { loadSettingsForAccount } = require("./settings-store");
      const settings = loadSettingsForAccount(next.waAccountId || "0488");
  ...
  /** قديم فقط — لا نعيد وسْم سجلات جديدة من عبدالرحمن إلى رايد */
  if (!next.waAccountId) {
