/**
 * إعدادات التواصل — ملف منفصل لكل جوال واتساب (رايد / عبدالرحمن / ماجد)
...
const FIELD_LABELS = {
  ...
  propertyComboAgentName: "الباقة — اسم المندوب",
  propertyComboAgentPhone: "الباقة — رقم المندوب",
...
const ACCOUNT_SEEDS = {
  ...
  wa_1780305984859: {
    ...
    branchEmployeePhone: "0531240724",
...
function getDefaults() {
  ...
  const hint =
    f.serviceStopContactHint || CONFIG.brand.contactHint || "من طرف رائد الحربي";
  const personalName = f.personalAgentName || f.employeeName;
  const personalPhone = f.personalAgentPhone || f.employeePhone;
  const comboFooter =
    f.propertyComboContactFooter || "من طرف رائد الحربي\nربي يسر أمرك";
  ...
  return {
    ...
    propertyComboAgentName: f.propertyComboAgentName || personalName,
    propertyComboAgentPhone: f.propertyComboAgentPhone || personalPhone,
...
function buildInitialStore() {
  ...
  if (!accounts["0488"]) {
    accounts["0488"] = legacy || seedForAccount("0488");
  }
  if (!accounts.wa_1780305984859) {
    accounts.wa_1780305984859 = seedForAccount("wa_1780305984859");
  }
  if (!accounts.majed) {
    accounts.majed = seedForAccount("majed");
  }
...
function saveSettingsForAccount(waAccountId, input) {
  const id = String(waAccountId || "").trim();
  if (!id) {
    throw new Error("يجب تحديد جوال واتساب (رايد أو عبدالرحمن أو ماجد)");
  }
