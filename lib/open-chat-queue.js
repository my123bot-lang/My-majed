/**
 * طابور فتح محادثة — ملف منفصل لكل جوال (رايد / عبدالرحمن / ماجد)
...
function botStartHint(accountId) {
  if (accountId === "0488") return "start-one-0488.bat أو start-three-whatsapp.bat";
  if (accountId === "wa_1780305984859") return "start-abdulrahman.bat";
...
async function waitForResult(id, timeoutMs = 40000, waAccountId) {
  ...
  throw new Error(
    `انتهى الوقت — تأكد أن نافذة بوت رايد/الجوال (${accountId}) مفتوحة و«جاهز» ثم شغّل: ${hint}`
  );
}
