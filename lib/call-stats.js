...
function normalizeStore(raw) {
  ...
  if (Object.keys(store.byAccount).length === 0 && raw.totals) {
    const raied = ensureAccount(store, "0488");
    mergeBucket(raied.totals, raw.totals);
    if (raw.daily) {
      for (const [day, bucket] of Object.entries(raw.daily)) {
        raied.daily[day] = emptyBucket();
        mergeBucket(raied.daily[day], bucket);
      }
    }
  }
  ...
  if (legacy) {
    const raied = ensureAccount(store, "0488");
    mergeBucket(raied.totals, legacy.totals);
    for (const [day, bucket] of Object.entries(legacy.daily || {})) {
      if (!raied.daily[day]) raied.daily[day] = emptyBucket();
      mergeBucket(raied.daily[day], bucket);
    }
  ...
  if (unknown) {
    const raied = ensureAccount(store, "0488");
    mergeBucket(raied.totals, unknown.totals);
