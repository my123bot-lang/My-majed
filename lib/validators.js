...
function normalizeText(text) {
  return normalizeDigits(String(text || ""))
    // علامات اتجاه ونص خفية يضيفها واتساب قبل التحية أو الرسالة المعاد توجيهها
    .replace(/[\u200B-\u200F\u202A-\u202E\u2060-\u2069\uFEFF]/g, "")
    .replace(/\u00A0/g, " ")
    .replace(/[\u064B-\u065F\u0670]/g, "")
    .replace(/\u0640/g, "")
...
/**
 * القائمة الرئيسية — 7 خيارات للعميل (الخطوة inquiry_type فقط).
 */
function parseInquiryType(text) {
  ...
  if (
    text === "7" ||
    text === "salary_policy" ||
    text.includes("سياسة الرواتب") ||
    text.includes("الرواتب المطلوبة") ||
    text.includes("رواتب مطلوبة")
  ) {
...
/** مدة التمويل بالسنوات (1–5) */
function parseLoanTermYears(text, allowedYears = [1, 2, 3, 4, 5]) {
  const allowed = new Set(
    (allowedYears || [1, 2, 3, 4, 5]).map((y) => Number(y))
  );
  ...
  const fromDigits = cleanNumber(t);
  if (allowed.has(fromDigits)) return fromDigits;

  const yearMatch = t.match(/(\d+)\s*(سنة|سنوات|سنين|سنه)/);
  if (yearMatch) {
    const years = Number(yearMatch[1]);
    if (allowed.has(years)) return years;
  }

  if (t.includes("سنتين") && allowed.has(2)) return 2;
  if ((t === "سنة" || t === "سنه" || t.includes("سنة واحدة")) && allowed.has(1)) {
    return 1;
  }
...
/**
 * يطابق كلمة مفتاحية كبداية للرسالة، لا تطابقاً حرفياً كاملاً — حتى ترد
 * على "مرحبا كيفكم" وليس فقط النص المطابق تماماً.
 * الكلمات الرقمية البحتة (مثل "1") تبقى مطابقة حرفية كاملة لتفادي الخلط مع أرقام الطلبات.
 */
function matchesStartKeyword(text, keywords) {
  const t = normalizeText(text).replace(/\s+/g, " ").trim();
  if (!t) return false;
  return (Array.isArray(keywords) ? keywords : []).some((kw) => {
    const k = String(kw || "").trim();
    if (!k) return false;
    if (/^\d+$/.test(k)) return t === k;
    return t === k || t.startsWith(k);
  });
}

/** تحية شائعة — تفتح القائمة لعميل جديد، ولا تعيد ضبط جلسة الحسبة. */
...
function isPoliteGreeting(text) {
  return matchesStartKeyword(text, POLITE_GREETINGS);
}
