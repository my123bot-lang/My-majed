...
/**
 * القدرة الشهرية المتاحة للتمويل بعد نسبة الاستقطاع والالتزامات.
 * نفس أساس معادلة المبلغ التقديري: (income × ratio) − commitments
 */
function getMonthlyFinancingCapacity(session) {
  const { ratios } = CONFIG.calculation;
  const realEstate = session?.realEstate || "none";
  let ratio = ratios.none;
  let income = Number(session?.salary) || 0;

  if (realEstate === "supported") {
    ratio = ratios.supported;
    income += Number(session?.supportAmount || 0);
  } else if (realEstate === "unsupported" || realEstate === "old") {
    ratio = ratios.unsupported;
  }

  const commitments =
    Number(session?.commitments || 0) +
    Number(session?.presetCommitmentDeduction || 0);

  return income * ratio - commitments;
}

/**
 * أعلى أصل قرض بحيث القسط الشهري ≤ maxInstallment (معادلة القطاع).
 */
function maxPrincipalForInstallment(
  maxInstallment,
  annualRatePercent,
  months = loanTermMonths,
  jobCategory = null
) {
  const cap = Number(maxInstallment);
  const m = Number(months);
  if (!cap || cap <= 0 || !m || m < 1) return 0;

  const formula = getInstallmentFormula(jobCategory);
  if (formula === "equalPrincipal") {
    const annual = Number(annualRatePercent) / 100;
    const perRiyal = 1 / m + annual / 12;
    if (!perRiyal || perRiyal <= 0) return 0;
    return Math.floor(cap / perRiyal);
  }

  // amortizing: بحث ثنائي
  let lo = 0;
  let hi = Math.max(Math.ceil(cap * m), 1);
  while (lo < hi) {
    const mid = Math.ceil((lo + hi) / 2);
    const inst = calculateMonthlyInstallment(
      mid,
      annualRatePercent,
      m,
      jobCategory
...
/**
 * يحدّ المبلغ التقديري بما يناسب القدرة الشهرية على أطول مدة مسموحة،
 * حتى لا يُعرض مبلغ ثم يُرفض عند اختيار السنوات.
 * @returns {number} مبلغ بعد التقريب (0 إن تعذّر)
 */
function capEstimatedAmountToAffordable(
  session,
  estimatedAmount,
  annualRatePercent,
  jobCategory = null,
  ...
  const yearsOptions = CONFIG.financing.loanTermYearsOptions || [1, 2, 3, 4, 5];
  const years =
    Number(maxYears) ||
    Math.max(...yearsOptions.map((y) => Number(y)).filter((y) => y > 0));
  if (!years || years < 1) return 0;

  const capacity = getMonthlyFinancingCapacity(session);
  // نفس هامش isLoanTermAffordable (+1)
  const maxPrincipal = maxPrincipalForInstallment(
    capacity + 1,
    annualRatePercent,
    years * 12,
    jobCategory
  );
  const capped = Math.min(Math.max(0, Number(estimatedAmount) || 0), maxPrincipal);
  let rounded = roundDownToStep(capped);

  // بعد التقريب لأسفل قد يبقى فوق الحد بسبب تقريب القسط — ننزّل خطوة
  let guard = 0;
  while (
    rounded > 0 &&
    ...
    const step =
      rounded >= (roundToTenThousandsMinAmount || 100000)
        ? roundToTenThousandsStep || 10000
        : lowerStep || 5000;
    rounded = roundDownToStep(Math.max(0, rounded - step));
    guard += 1;
  }
...
function isLoanTermAffordable(
  ...
  const capacity = getMonthlyFinancingCapacity(session);
  // هامش ريال واحد لتقريب القسط
  return installment <= capacity + 1;
}

/**
 * كل المدد المسموحة (بالسنوات) بحيث القسط يناسب القدرة الشهرية.
 * @returns {number[]}
 */
function listAffordableLoanTermYears(
  session,
  amount,
  annualRatePercent,
  jobCategory = null,
  ...
  const yearsList = (options || [1, 2, 3, 4, 5])
    .map((y) => Number(y))
    .filter((y) => y > 0)
    .sort((a, b) => a - b);

  return yearsList.filter((years) =>
    isLoanTermAffordable(
      session,
      amount,
      years,
...
/**
 * أعلى مدة مسموحة (بالسنوات) بحيث القسط يناسب القدرة الشهرية.
 * @returns {number|null}
 */
function resolveMaxAffordableLoanTermYears(
  session,
  amount,
  annualRatePercent,
  jobCategory = null,
