...
async function sendOneInteractiveList(
  ...
  const { countryCode, phoneNumber } = splitPhone(phone, cfg.countryCode);
  const body = String(bodyText || "").trim().slice(0, 1024);
  const listRows = (rows || []).slice(0, 10).map((r, i) => ({
    ...
    data: {
      message: {
        ...
        action: {
          ...
          sections: [{ title: "القائمة", rows: listRows }],
        },
      },
    },
  });
...
async function sendInteractiveViaInterakt(phone, menu) {
  ...
  if (menu.kind === "buttons") {
    return sendOneInteractiveButton(phone, menu.body, menu.buttons);
  }
  if (menu.kind === "list") {
    return sendOneInteractiveList(
      phone,
      menu.body,
...
async function listUsersPage({ offset = 0, limit = 100, sinceIso } = {}) {
  ...
  if (sinceIso) {
    body.filters = [
      { trait: "created_at_utc", op: "gt", val: sinceIso },
    ];
  }
