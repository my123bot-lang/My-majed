 ...
 */
/**
 * قوالب واتساب ترفض الأسطر الجديدة/التبويب وأكثر من مسافتين متتاليتين داخل المتغيرات.
 */
function sanitizeTemplateVariable(text) {
  return String(text || "")
    .replace(/\r\n/g, "\n")
    .replace(/\t/g, " ")
    .replace(/\n+/g, " | ")
...
async function sendTemplateReply(phone, text, callbackData = "bot_reply") {
  ...
  const { countryCode, phoneNumber } = splitPhone(phone, cfg.countryCode);
  const body = sanitizeTemplateVariable(text);
  if (!body) {
    return { ok: false, error: "نص فارغ بعد التنظيف" };
  }
