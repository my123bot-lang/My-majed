...
async function tryHandleCustomerChatControl(msg) {
  ...
  const extraKeys = sessionStore.findRelatedIdentityKeys(...seedKeys);

  if (cmd === "stop") {
    autoReplyControl.pauseChat(from, { extraKeys });
    sessionStore.clearSessionsMatching(from, ...extraKeys);
  ...
  await replyToMessage(msg, CONFIG.botControl.customerResumedReply);
  return true;
}
