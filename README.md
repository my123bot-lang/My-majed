...
### إيقاف عميل واحد واستئنافه (أنت فقط — ليس العميل)

| الأمر | النتيجة |
|-------|---------|
| زر **إيقاف الرد / تشغيل الرد** في سجل العملاء | إيقاف/استئناف الرد الآلي لهذا العميل |
| `stop` / `إيقاف` من **رقم التحكم** إلى رقم البوت | إيقاف الرد الآلي لآخر عميل (أو `stop 05xxxxxxxx`) |
| `start` / `تشغيل` من **رقم التحكم** | استئناف الرد الآلي |
| فتح محادثة العميل من اللوحة | يوقف الرد الآلي لهذا العميل تلقائياً |
| قائمة البوت من **رقم التحكم** | أرسل `مرحبا` أو `تحكم` — يظهر لك فقط أزرار إيقاف/تشغيل |
| أي رد يدوي منك | يوقف الرد الآلي لهذا العميل تلقائياً (إن وصل الحدث للخادم) |
| رسالة `stop` من **العميل** | لا أثر — العميل لا يتحكم بالرد الآلي |
