...
9:00 صباحاً — 5:00 مساءً`,
    locationInfo: `موقعنا/حائل معرض السديري للسيارات
عند حضورك اسأل عن الموظف ماجد`,
  ...
  financing: {
    ...
    loanTermMonths: 60,
    /** خيارات مدة التمويل بالسنوات — تُعرض فقط بعد اختيار مبلغ أقل */
  ...
  // ---------------------------------------------------------------------------
  // أوامر stop / start — من العميل (وارد) أو من حسابك (صادر fromMe)
  // ---------------------------------------------------------------------------
  botControl: {
    ...
    /** رد للعميل عند إرسال start */
    customerResumedReply:
      "تم استئناف الرد الآلي على هذه المحادثة.\n\nأرسل مرحبا للمتابعة.",
    pausedReply: "تم إيقاف الرد الآلي على جميع العملاء.",
    resumedReply:
      "تم استئناف الرد الآلي على جميع العملاء.\n\n(شمل المحادثات التي أوقفت بـ stop)",
  },
  ...
  session: {
    ...
    spamDelayMs: 1200,
    /**
     * كلمات إعادة ضبط الجلسة من أول خطوة (أي خطأ في الإدخال).
     * تُطابق أيضاً إعاده/اعاده بعد توحيد ة→ه وأ/إ→ا.
     */
    resetKeywords: ["reset", "إعادة", "اعادة", "إعاده", "اعاده"],
    /**
     * كلمات إظهار القائمة الرئيسية فقط — تُطابق كبداية للرسالة لا نصاً مطابقاً بالكامل،
     * حتى ترد على "السلام عليكم ورحمة الله وبركاته" أو "هلا وغلا" لا فقط النص الحرفي.
     * «1» يفتح القائمة خارج الجلسة النشطة فقط (لا يعيد الضبط وسط الحسبة) ويبقى تطابقاً كاملاً.
     */
    restartKeywords: ["مرحبا", "هلا", "أهلا", "اهلا", "السلام عليكم", "وعليكم السلام"],
    menuStartKeywords: ["مرحبا", "هلا", "أهلا", "اهلا", "السلام عليكم", "وعليكم السلام", "1"],
    /**
     * بعد إرسال الرابط أو إتمام التقديم — لا تُعاد القائمة تلقائياً بعد عدد رسائل؛
     * القائمة تظهر فقط بكلمات menuStartKeywords.
     */
    postCloseMenuThreshold: 9999,
    /**
...
نأسف لعدم تقديمكم للطلب`,
  ...
  messages: {
    ...
    start: `وعليكم السلام ورحمه الله وبركاته
...
6- خدمات مابعد البيع`,
    ...
    afterSalesEarlyPayoff: `تم اختيار: سداد مبكر
...
3- رقم الجوال المسجل لدينا`,
    ...
    afterSalesCollectionIssue: `تم اختيار: مشكلة بالتحصيل
...
3- اشرح مشكلتك`,
    ...
    afterSalesComplaint: `تم اختيار: شكاوى
...
2- قطاع خاص`,
    ...
    companySearchPrompt: `أرسل اسم الشركة للبحث في قائمة الشركات المعتمدة
أو جزء منها.

مثال:
ارامكس، المراعي، ارامكو، سابك، بنك الاهلي`,

    companyNotFound: `نعتذر منك، الشركة غير موجودة ضمن قائمة الشركات المعتمدة.

أعد كتابة اسم الشركة بشكل أوضح، أو اكتب: مرحبا للعودة للقائمة.`,
...
للقائمة الرئيسية اكتب: مرحبا أو هلا أو السلام عليكم أو 1`,
...
إذا حطيت رقم خطأ أرسل كلمة: إعادة`,
...
إعادة المحاولة متاحة مرة واحدة فقط.`,
    ...
    lowerAmountQuestion: `هل ترغب بمبلغ تمويل أقل؟
...
رقم التواصل: ${employeePhone}`;
    ...
    branchVisitInfo: () =>
      `موقعنا/حائل معرض السديري للسيارات
عند حضورك اسأل عن الموظف ماجد`,
